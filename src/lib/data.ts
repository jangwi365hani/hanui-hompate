...
export interface Doctor {
  id: string;          // 고유 id (기본값은 slug)
  slug: string;        // URL 슬러그 (예: "kim")
  name: string;        // 이름
  title: string;       // 직책 (예: 대표원장, 원장)
  subtitle: string;    // 한 줄 소개 문구
  tags: string[];      // 전문 분야 태그
  bio: string[];       // 원장 인사말 (문단 배열)
  imageUrl: string;    // 프로필 사진 (기본 /images/*.png 또는 업로드 URL)
  isActive: boolean;   // 홈페이지 노출 여부
}

// 블롭에 저장된 의료진 데이터가 없을 때 사용하는 기본값.
// (기존 하드코딩 데이터를 그대로 옮긴 것)
export const DEFAULT_DOCTORS: Doctor[] = [
  ...
  },
  {
    ...
    name: "김경민",
    title: "원장",
    subtitle: "편하게 이야기하고, 함께 풀어가는 진료",
    tags: ["소통 중심 진료", "맞춤 치료", "생활습관 관리"],
    bio: [
      "안녕하세요. 장위365경희한의원 김경민 원장입니다.",
      "살다보면 한 번쯤은 \"왜 계속 아프지\"라는 생각이 드는 순간이 있습니다. 병원에서는 큰 이상이 없다고 하는데, 몸은 분명히 불편한 그런 경험이요.",
      "저는 그런 증상들에는 다 이유가 있다고 생각합니다. 반복되는 생활 패턴, 누적된 피로와 스트레스처럼 눈에 잘 보이지 않는 것들이 조금씩 쌓여 몸의 균형을 무너뜨리고, 결국 통증이나 불편함으로 나타나게 됩니다.",
      "그래서 저는 진료를 할 때 환자분과 함께 '이 증상이 왜 생겼을까'를 고민하는 데 집중합니다. 환자분의 이야기를 충분히 듣고, 몸의 흐름을 하나씩 짚어보며 그 원인을 찾아가는 과정이 치료의 시작이라고 생각합니다.",
      "환자마다 통증의 양상과 회복 속도는 모두 다릅니다. 그렇기 때문에 획일적인 치료보다는 환자분 개개인의 상태에 맞춘 진료가 필요합니다. 그날의 몸 상태에 가장 적합한 방법을 선택해 무리가 가지 않으면서도 효과적으로 회복할 수 있도록 도와드리겠습니다.",
      "저는 환자분이 자신의 몸을 이해하고, 치료 과정에 함께 참여하고 있다고 느낄 수 있는 진료를 지향합니다. 편하게 이야기할 수 있는 분위기 속에서 몸의 문제를 하나씩 풀어가다 보면, 결국 가장 자연스럽고 지속적인 회복으로 이어진다고 믿습니다.",
      "환자분의 이야기에 귀 기울이며, 소통을 바탕으로 신뢰할 수 있는 진료를 이어가겠습니다.",
    ],
    imageUrl: "/images/doctorm.png",
    isActive: true,
  },
];
...
export async function getDoctors(): Promise<Doctor[]> {
  try {
    const data = await fetchLatestBlob("hanui-doctors");
    return (data as Doctor[]) ?? DEFAULT_DOCTORS;
  } catch {
